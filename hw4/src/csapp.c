/*
 *
 * Modified  10/2017 by WEIFENG LIN : Other Wrapper Functions added commented with "MODIFIED"
 *
...
void Execve(const char *filename, char *const argv[], char *const envp[])
{
    if (execve(filename, argv, envp) < 0){
        ...
        {
            printf(EXEC_NOT_FOUND,"Execve error");
        }else{
            printf(EXEC_ERROR, "Execve error");
        }
...
int sf_chdir(const char *path){
    //printf("sf_chdir: path: %s\n", path);
    int ret = chdir(path);
    if (ret == -1)
    {
        printf(BUILTIN_ERROR, "Error in changing directory.");
    }

    return ret;
}

/*Caller of this function must free the buf.*/
char* sf_getcwd(){
    size_t size = 512;
